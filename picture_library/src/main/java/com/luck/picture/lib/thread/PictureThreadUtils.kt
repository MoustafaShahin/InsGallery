...
object PictureThreadUtils {
    ...
    private val TASK_TASKINFO_MAP: MutableMap<Task<*>, TaskInfo?> = ConcurrentHashMap()
    ...
    fun getSinglePool(): ExecutorService? {
        return getPoolByTypeAndPriority(TYPE_SINGLE.toInt())
    }
    ...
    fun getCachedPool(): ExecutorService? {
        return getPoolByTypeAndPriority(TYPE_CACHED.toInt())
    }
    ...
    fun getIoPool(): ExecutorService? {
        return getPoolByTypeAndPriority(TYPE_IO.toInt())
    }
    ...
    fun getCpuPool(): ExecutorService? {
        return getPoolByTypeAndPriority(TYPE_CPU.toInt())
    }
    ...
    fun <T> executeBySingleWithDelay(task: Task<T>,
                                     delay: Long,
                                     unit: TimeUnit) {
    ...
    fun <T> executeBySingleAtFixRate(task: Task<T>,
                                     period: Long,
                                     unit: TimeUnit) {
    ...
    fun <T> executeByCachedWithDelay(task: Task<T>,
                                     delay: Long,
                                     unit: TimeUnit) {
    ...
    fun <T> executeByCachedAtFixRate(task: Task<T>,
                                     period: Long,
                                     unit: TimeUnit) {
    ...
    fun <T> executeByIoWithDelay(task: Task<T>,
                                 delay: Long,
                                 unit: TimeUnit) {
    ...
    fun <T> executeByIoAtFixRate(task: Task<T>,
                                 period: Long,
                                 unit: TimeUnit) {
    ...
    fun <T> executeByCpuWithDelay(task: Task<T>,
                                  delay: Long,
                                  unit: TimeUnit) {
    ...
    fun <T> executeByCpuAtFixRate(task: Task<T>,
                                  period: Long,
                                  unit: TimeUnit) {
    ...
    internal class ThreadPoolExecutor4Util(corePoolSize: Int, maximumPoolSize: Int,
        ...
        private fun getSubmittedCount(): Int {
    ...
    private class LinkedBlockingQueue4Util : LinkedBlockingQueue<Runnable> {
        @Volatile
        val mPool: ThreadPoolExecutor4Util? = null
    ...
    private class UtilsThreadFactory @JvmOverloads internal constructor(prefix: String, priority: Int, isDaemon: Boolean = false) : AtomicLong(), ThreadFactory {

 ...
 * 使用 [.applyInstagramOptions] 方法即可快速应用 InsGallery 需要的所有设置
 *
 *
 ...
 * ================================================
 */
class InsGallery private constructor() {
    companion object {
        ...
        fun openGallery(activity: Activity, engine: ImageEngine?, listener: OnResultCallbackListener?) {
            openGallery(activity, engine, null, null, listener)
        }

        fun openGallery(activity: Activity, engine: ImageEngine?, cacheResourcesEngine: CacheResourcesEngine?, listener: OnResultCallbackListener?) {
            openGallery(activity, engine, cacheResourcesEngine, null, listener)
        }

        fun openGallery(activity: Activity, engine: ImageEngine?, cacheResourcesEngine: CacheResourcesEngine?, selectionMedia: List<LocalMedia?>?, listener: OnResultCallbackListener?) {
        ...
        fun openGallery(activity: Activity, engine: ImageEngine?, cacheResourcesEngine: CacheResourcesEngine?, selectionMedia: List<LocalMedia?>?, instagramConfig: InstagramSelectionConfig?, listener: OnResultCallbackListener?) {
        ...
        fun openGallery(activity: Activity, engine: ImageEngine?, cacheResourcesEngine: CacheResourcesEngine?, selectionMedia: List<LocalMedia?>?) {
            applyInstagramOptions(activity.getApplicationContext(), PictureSelector.create(activity)
                    .openGallery(PictureMimeType.ofAll())) // 全部.PictureMimeType.ofAll()、图片.ofImage()、视频.ofVideo()、音频.ofAudio()
                    .imageEngine(engine) // 外部传入图片加载引擎，必传项
                    .loadCacheResourcesCallback(cacheResourcesEngine) // 获取图片资源缓存，主要是解决华为10部分机型在拷贝文件过多时会出现卡的问题，这里可以判断只在会出现一直转圈问题机型上使用
        ...
        fun openGallery(activity: Activity, engine: ImageEngine?, cacheResourcesEngine: CacheResourcesEngine?, selectionMedia: List<LocalMedia?>?, instagramConfig: InstagramSelectionConfig?) {
            applyInstagramOptions(activity.getApplicationContext(), instagramConfig, PictureSelector.create(activity)
                    .openGallery(PictureMimeType.ofAll())) // 全部.PictureMimeType.ofAll()、图片.ofImage()、视频.ofVideo()、音频.ofAudio()
                    .imageEngine(engine) // 外部传入图片加载引擎，必传项
                    .loadCacheResourcesCallback(cacheResourcesEngine) // 获取图片资源缓存，主要是解决华为10部分机型在拷贝文件过多时会出现卡的问题，这里可以判断只在会出现一直转圈问题机型上使用
        ...
        fun openGallery(activity: Activity, engine: ImageEngine?, cacheResourcesEngine: CacheResourcesEngine?, selectionMedia: List<LocalMedia?>?, requestCode: Int) {
            applyInstagramOptions(activity.getApplicationContext(), PictureSelector.create(activity)
                    .openGallery(PictureMimeType.ofAll())) // 全部.PictureMimeType.ofAll()、图片.ofImage()、视频.ofVideo()、音频.ofAudio()
                    .imageEngine(engine) // 外部传入图片加载引擎，必传项
                    .loadCacheResourcesCallback(cacheResourcesEngine) // 获取图片资源缓存，主要是解决华为10部分机型在拷贝文件过多时会出现卡的问题，这里可以判断只在会出现一直转圈问题机型上使用
        ...
        fun applyInstagramOptions(context: Context, selectionModel: PictureSelectionModel): PictureSelectionModel {
        ...
        fun applyInstagramOptions(context: Context, instagramConfig: InstagramSelectionConfig?, selectionModel: PictureSelectionModel): PictureSelectionModel {
        ...
        fun setCurrentTheme(currentTheme: Int) {
            Companion.currentTheme = currentTheme
        }
    }
